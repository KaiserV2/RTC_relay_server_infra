...
```bash
curl -sSL https://raw.githubusercontent.com/Froot-NetSys/RTC_relay_server_infra/main/vpn-proxy/install.sh | bash
```
...
```bash
sudo git clone https://github.com/Froot-NetSys/RTC_relay_server_infra.git /opt/rtcproxy
cd /opt/rtcproxy
...
$VM1_IP    = "20.55.35.218"
$VM2_IP    = "20.24.57.248"
$SSH_KEY   = "C:\Users\USER\.ssh\azure_rtc.pem"
$USER      = "azureuser"
$BASE_DIR  = "C:\Users\USER\captures"
...
...
===================================
Captured files stored in: C:\Users\USER\captures\west-europe-south-east-asia-run_5
```
...
* Output and logs are stored per run under:
  ...
  ```
  C:\Users\USER\captures\<region1>-<region2>-run_<N>\
  ```

* **For business/paid tier accounts** (e.g., Zoom Business, Microsoft Teams), you must provision your own subscriptions. Paid-tier coverage is relevant because some RTC providers restrict relay data-centre selection to paid accounts (see the paper for details).
