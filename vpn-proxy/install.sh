...
if [ -f "${SCRIPT_DIR}/api.py" ] && [ -f "${SCRIPT_DIR}/requirements.txt" ]; then
	...
	if [ ! -f /opt/rtcproxy/api.py ]; then
		...
		sudo -u "${SERVICE_USER}" git clone https://github.com/Froot-NetSys/RTC_relay_server_infra.git /opt/rtcproxy-src \
			&& sudo -u "${SERVICE_USER}" cp -r /opt/rtcproxy-src/vpn-proxy/. /opt/rtcproxy/ \
			&& rm -rf /opt/rtcproxy-src
	fi
fi
